...
{
    protected $_validate = array(array('name', 'require', '{%tg_error_null_name}'), array('name', '', '{%tg_error_has_name}', 0, 'unique', 1),);

    public function getLevelList()
    {
        static $list = null;
        if (!isset($list)) {
            $level1List = $this->where('`parent_id`=0')->order('`sort` desc')->select();
            $level1Ids = array();
            ...
            $subList = $this->where($cond)->order('`sort` desc')->select();
    ...
    public function getLevelSelect($sltName = 'tg_id')
    {
        ...
        $html .= '<option value="0">' . L('none') . '</option>';
