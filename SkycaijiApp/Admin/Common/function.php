...
function program_filemd5_list($path, &$md5FileList)
{
    ...
    if (empty($passPaths)) {
        $passPaths['data'] = realpath(C('ROOTPATH') . '/data');
        $passPaths['runtime'] = realpath(C('ROOTPATH') . '/' . APP_PATH . 'Runtime');
...
{
    if ($GLOBALS['usertoken'] != I('_usertoken_')) {
        return false;
    } else {
        return true;
    }
