...
class DevelopController extends BaseController
{
    ...
    public function releaseCmsAction()
    {
        ...
        if (IS_POST) {
            ...
            $name = I('name');
            if (empty($name)) {
                $this->error('请输入发布应用名称');
            }
            if (!$is_edit) {
                $cms_name = I('cms_name');
                $cms_name_custom = I('cms_name_custom');
                ...
                if (empty($identifier)) {
                    $this->error('请输入应用功能标识');
                } elseif (!preg_match('/^[a-z][a-z0-9]*$/i', $identifier)) {
                    $this->error('应用功能标识必须由字母或数字组成且首位不能为数字！');
                }
                if (empty($copyright)) {
                    $this->error('请输入作者版权');
            ...
            if ($appName) {
                ...
                if (!empty($cmsData)) {
                    ...
                    if (preg_match('/^([A-Z][a-z0-9]*)([A-Z][a-z0-9]*)([A-Z][a-z0-9]*)$/', $appName, $appInfo)) {
                        ...
                        $config['app_file'] = realpath(C('ROOTPATH') . '/' . APP_PATH . 'Release/Cms/' . $appName . 'Cms.class.php');
                    }
                    $config['params'] = array();
                    try {
                        $cmsClass = $this->get_cms_class($appName);
                    } catch (\Exception $ex) {
                        $this->error($ex->getMessage());
                    }
                    foreach ($cmsClass->_params as $k => $v) {
                        $param = array('key' => $k, 'require' => intval($v['require']), 'name' => $v['name'],);
                        if ($v['tag'] == 'select') {
                            if ($v['option'] == 'function:param_option_fields') {
            ...
            $GLOBALS['content_header'] = '开发CMS发布应用 <small><a href="http://www.skycaiji.com/manual/doc/cms" target="_blank"><span class="glyphicon glyphicon-info-sign"></span></a></small>';
            $GLOBALS['breadcrumb'] = breadcrumb(array('开发工具', '开发CMS发布应用'));
            $this->assign('config', $config);
            $this->display('releaseCms');
        }
    }
    ...
    public function create_cms_app($appData, $params, $is_edit = false)
    {
        ...
        if (!$is_edit && !empty($cmsData)) {
            $this->error('抱歉，已存在' . $appData['app'] . '应用');
        }
        $_params = array();
        $newFuncs = array();
        foreach ($params as $k => $v) {
            $pkey = $v['key'];
            $_params[$pkey] = array('name' => $v['name'], 'require' => intval($v['require']));
        ...
        $cmsClass = $this->get_cms_class($appData['app']);
        $existsFuncs = array();
        if (!empty($cmsClass)) {
...
EOF;
            ...
            $phpCode = file_get_contents(C('ROOTPATH') . '/' . APP_PATH . 'Release/Cms/' . $appData['app'] . 'Cms.class.php');
            $phpCode = preg_replace('/public\s*\$_params\=[\s\S]+?\);/i', 'public $_params=' . $_params . ';', $phpCode);
            if (!empty($funcPhp)) {
        ...
        if (!empty($phpCode)) {
            ...
            if ($success) {
                $this->success('创建成功', U('Develop/releaseCms?app=' . $appData['app']));
            } else {
                $this->error('创建失败');
            }
        } else {
    ...
    public function get_cms_class($appName)
    {
        $appName = ucfirst($appName);
        $appPath = C('ROOTPATH') . '/' . APP_PATH . 'Release/Cms/' . $appName . 'Cms.class.php';
        if (file_exists($appPath)) {
            $cmsClass = 'Release\\Cms\\' . $appName . 'Cms';
