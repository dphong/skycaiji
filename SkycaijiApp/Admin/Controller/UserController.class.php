...
class UserController extends BaseController
{
    public function listAction()
    {
        ...
        $limit = 20;
        $count = $muser->count();
        $userList = $muser->order('uid asc')->limit($limit)->page($page)->select();
        if ($count > $limit) {
        ...
        $GLOBALS['content_header'] = L('user_list');
        $GLOBALS['breadcrumb'] = breadcrumb(array(array('url' => U('User/list'), 'title' => L('user_list'))));
        $groupList = D('Usergroup')->select(array('index' => 'id'));
    ...
    public function addAction()
    {
        ...
        if (IS_POST) {
            ...
            $newData = array('username' => I('username'), 'password' => I('password'), 'repassword' => I('repassword'), 'email' => I('email'), 'groupid' => I('groupid', 0, 'intval'));
            ...
            $uid = $muser->add($newData);
            if ($uid > 0) {
                $this->success(L('op_success'), U('User/list'));
            } else {
                $this->error(L('op_failed'));
            }
        } else {
            $subGroupList = $musergroup->get_sub_level($GLOBALS['user']['groupid']);
            $GLOBALS['content_header'] = L('user_add');
            $GLOBALS['breadcrumb'] = breadcrumb(array(array('url' => U('User/list'), 'title' => L('user_list')), L('user_add')));
    ...
    public function editAction()
    {
        $uid = I('uid', 0, 'intval');
        if (empty($uid)) {
            $this->error(L('user_error_null_uid'));
        }
        $muser = D('User');
        $musergroup = D('Usergroup');
        $userData = $muser->getByUid($uid);
        if (empty($userData)) {
            $this->error(L('user_error_empty_user'));
        }
        ...
        if (IS_POST) {
            ...
            $newData = array('password' => I('password'), 'repassword' => I('repassword'), 'email' => I('email'), 'groupid' => I('groupid', 0, 'intval'));
            ...
            $muser->where(array('uid' => $uid))->save($newData);
            $this->success(L('op_success'), U('User/list'));
        } else {
            ...
            $GLOBALS['content_header'] = L('user_edit');
            $GLOBALS['breadcrumb'] = breadcrumb(array(array('url' => U('User/list'), 'title' => L('user_list')), L('user_edit')));
            $this->display();
        }
    }
