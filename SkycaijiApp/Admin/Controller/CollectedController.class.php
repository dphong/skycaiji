...
class CollectedController extends BaseController
{
    public function listAction()
    {
        ...
        if (!empty($taskName)) {
            ...
            $searchTasks = $mtask->field('`id`,`name`')->where(array('name' => array('like', "%{$taskName}%")))->select(array('index' => 'id,name'));
        ...
        $search['num'] = I('num', 200, 'intval');
        $search['url'] = I('url');
        if (!empty($search['url'])) {
            $cond['url'] = array('like', '%' . addslashes($search['url']) . '%');
        }
        $search['release'] = I('release');
        ...
        if (!empty($search['status'])) {
            if ($search['status'] == 1) {
                $cond['target'] = array('EXP', "!=''");
            } elseif ($search['status'] == 2) {
                $cond['error'] = array('EXP', "!=''");
            }
        }
        if (!$null_task) {
            ...
            $limit = $search['num'];
            if ($count > 0) {
                $dataList = $mcollected->where($cond)->order('id desc')->limit($limit)->page($page)->select();
                if ($count > $limit) {
                ...
                if (!empty($taskIds)) {
                    $taskList = D('Task')->where(array('id' => array('in', $taskIds)))->select(array('index' => 'id,name'));
                }
                $this->assign('dataList', $dataList);
                $this->assign('taskList', $taskList);
            }
            $GLOBALS['content_header'] = L('collected_list');
            $GLOBALS['breadcrumb'] = breadcrumb(array(array('url' => U('Collected/list'), 'title' => L('collected_list'))));
        }
